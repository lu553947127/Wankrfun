 ...
 * @Package: com.wankrfun.crania.view.mine
 * @ClassName: MineApplyFragment
 * @Description: 我的申请fragment
 * @Author: 鹿鸿祥
 * @CreateDate: 1/28/21 9:55 AM
 * @UpdateUser: 更新者
 * @UpdateDate: 1/28/21 9:55 AM
 * @UpdateRemark: 更新说明
 * @Version: 1.0
 */
public class MineApplyFragment extends BaseLazyFragment {
    ...
    protected void initDataAndEvent(Bundle savedInstanceState) {
        tvApply = mActivity.findViewById(R.id.tv_apply);
        ...
        //获取用户发布的活动信息
        mineViewModel.userEventsListAppliedLiveData.observe(this, userEventsListBean -> {
            tvApply.setText("我的申请 • " + (userEventsListBean.getUserEventsListBean().getData().getTotal()));
            if (userEventsListBean.getUserEventsListBean().getData().getActiveList().size() == 0 && userEventsListBean.getList().size() == 0){
                tvEmpty.setText("你还没有申请的活动");
    ...
    protected void initDataFromService() {
        mineViewModel.getUserEventsListApplied(SPUtils.getInstance().getString(SpConfig.USER_ID));
    }
}
