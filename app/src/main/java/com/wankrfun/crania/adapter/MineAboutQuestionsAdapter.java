 ...
 * @Package: com.wankrfun.crania.adapter
 * @ClassName: MineAboutQuestionsAdapter
 * @Description: java类作用描述
 * @Author: 鹿鸿祥
 * @CreateDate: 4/23/21 1:16 PM
 * @UpdateUser: 更新者
 * @UpdateDate: 4/23/21 1:16 PM
 * @UpdateRemark: 更新说明
 * @Version: 1.0
 */
public class MineAboutQuestionsAdapter extends BannerAdapter<List<QuestionListBean.DataBean.ListBean>, MineAboutQuestionsAdapter.BannerViewHolder> {
    public Context context;
    public MineAboutQuestionsAdapter(Context context, List<List<QuestionListBean.DataBean.ListBean>> beanList) {
    ...
    @Override
    public MineAboutQuestionsAdapter.BannerViewHolder onCreateHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_mine_about_questions, parent,false);
        return new MineAboutQuestionsAdapter.BannerViewHolder(view);
    }

    @Override
    public void onBindView(MineAboutQuestionsAdapter.BannerViewHolder holder, List<QuestionListBean.DataBean.ListBean> item, int position, int size) {
        switch (item.size()){
            case 1:
